...
(function () {
    ...
    if (markPaidBtn) {
        markPaidBtn.addEventListener('click', async function () {
            const orderId = this.getAttribute('data-order-id');
            const confirmed = confirm('Are you sure you want to mark this order as PAID? This will record a manual payment.');
            if (!confirmed) return;

            const notes = prompt('Enter payment notes (optional):', 'Manual payment confirmation');
            ...
            try {
                ...
                const siteUrl = window.SITE_URL || '';
                const response = await fetch(siteUrl + '/admin/mark-order-paid', {
                    method: 'POST',
                    body: formData
                });
    ...
    if (refundForm) {
        refundForm.addEventListener('submit', async function (e) {
            ...
            try {
                const formData = new FormData(this);
                const siteUrl = window.SITE_URL || '';
                const response = await fetch(siteUrl + '/admin/process-order-refund', {
                    method: 'POST',
                    body: formData
                });
