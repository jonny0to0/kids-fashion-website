...
<div class="mb-6">
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 md:gap-0 mb-4">
        <div>
        ...
        <form method="GET" action="<?php echo SITE_URL; ?>/admin/analytics"
            class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
            <select name="days"
                class="w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 text-sm">
                <option value="7" <?php echo $days == 7 ? 'selected' : ''; ?>>Last 7 Days</option>
                <option value="30" <?php echo $days == 30 ? 'selected' : ''; ?>>Last 30 Days</option>
            </select>
            <button type="submit"
                class="w-full sm:w-auto btn-pink-gradient px-6 py-2 rounded-lg font-medium text-sm text-center justify-center">Apply</button>
        </form>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="admin-card">
            ...
            <?php if ($revenueGrowth != 0): ?>
                <p class="text-xs mt-1 <?php echo $revenueGrowth > 0 ? 'text-green-600' : 'text-red-600'; ?>">
                    <?php echo $revenueGrowth > 0 ? '↑' : '↓'; ?>     <?php echo number_format(abs($revenueGrowth), 1); ?>% vs
                    previous period
                </p>
            <?php endif; ?>
        </div>
        ...
        <div class="admin-card">
            ...
            <p class="text-2xl font-bold text-gray-800 mt-2">
                <?php echo $totalOrders > 0 ? number_format(($totalOrders / max($totalOrders, 1)) * 100, 1) : '0'; ?>%
            </p>
    ...
    <div class="admin-card bg-gradient-to-r from-pink-50 to-purple-50 border border-pink-200">
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">Access advanced filters, export options, and detailed analysis in
                    Revenue Analytics</p>
            </div>
            <a href="<?php echo SITE_URL; ?>/admin/revenue-analytics"
                class="w-full md:w-auto btn-pink-gradient px-6 py-2.5 rounded-lg font-medium inline-flex items-center justify-center gap-2">
