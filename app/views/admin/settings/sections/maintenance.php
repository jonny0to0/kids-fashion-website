...
<div class="space-y-6">
    ...
    <!-- Current Status -->
    <div class="bg-white border-2 <?php echo $maintenanceEnabled ? 'border-red-300 bg-red-50' : 'border-green-300 bg-green-50'; ?> rounded-lg p-4">
        <div class="flex items-center justify-between">
            <div>
                <h3 class="text-lg font-semibold <?php echo $maintenanceEnabled ? 'text-red-800' : 'text-green-800'; ?> mb-1">
    ...
    <input type="hidden" name="settings[maintenance_mode_enabled]" 
    ...
    <div>
        ...
        <textarea name="settings[maintenance_message]" 
        ...
        <p class="text-xs text-gray-500 mt-1">This message will be shown to users. Keep it friendly and non-technical.</p>
    </div>
    ...
    <div>
        ...
        <input type="text" 
               name="settings[maintenance_reason]" 
               id="maintenance_reason"
               value="<?php echo htmlspecialchars($maintenanceReason); ?>"
               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent"
               placeholder="Scheduled maintenance">
        <p class="text-xs text-gray-500 mt-1">Short, user-friendly reason (e.g., "Scheduled maintenance", "Performance upgrade")</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
            ...
            <input type="datetime-local" 
                   name="settings[maintenance_end_time]" 
    ...
    <div>
        ...
        <input type="number" 
               name="settings[maintenance_auto_disable_after]" 
               id="maintenance_auto_disable_after"
               value="<?php echo $autoDisableHours; ?>"
               min="0"
               step="1"
               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent"
               placeholder="0 = disabled">
        <p class="text-xs text-gray-500 mt-1">Automatically disable maintenance after X hours (0 = disabled). Recommended: 6-12 hours as fail-safe.</p>
    </div>
    ...
    <div>
        ...
        <input type="email" 
               name="settings[maintenance_support_email]" 
               id="maintenance_support_email"
               value="<?php echo htmlspecialchars($supportEmail); ?>"
               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent"
               placeholder="support@example.com">
        <p class="text-xs text-gray-500 mt-1">Email address shown on maintenance page (falls back to general support email if empty)</p>
    </div>
    ...
    <div>
        ...
        <input type="url" 
               name="settings[maintenance_status_page_url]" 
               id="maintenance_status_page_url"
    ...
    <div>
        ...
        <input type="text" 
               name="settings[maintenance_allowed_ips]" 
               id="maintenance_allowed_ips"
               value="<?php echo htmlspecialchars($allowedIPs); ?>"
               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent"
               placeholder="192.168.1.1, 10.0.0.1">
        <p class="text-xs text-gray-500 mt-1">Comma-separated IP addresses that can access the site during maintenance (in addition to admins)</p>
    </div>

    <!-- Info Box -->
    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <h3 class="text-sm font-semibold text-blue-800 mb-2">Maintenance Mode Best Practices</h3>
