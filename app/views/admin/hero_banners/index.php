...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-lg shadow-md p-4 mb-6">
        <form method="GET" action="<?php echo SITE_URL; ?>/admin/hero-banners" class="flex gap-4 flex-wrap">
            ...
            <select name="target_type" class="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500">
                <option value="">All Types</option>
                <option value="homepage" <?php echo ($filters['target_type'] ?? '') === 'homepage' ? 'selected' : ''; ?>>Homepage</option>
                <option value="category" <?php echo ($filters['target_type'] ?? '') === 'category' ? 'selected' : ''; ?>>Category</option>
                <option value="campaign" <?php echo ($filters['target_type'] ?? '') === 'campaign' ? 'selected' : ''; ?>>Campaign</option>
            </select>
            <button type="submit" class="bg-gray-600 text-white px-6 py-2 rounded-lg hover:bg-gray-700">
                Filter
            </button>
            <?php if (!empty($filters)): ?>
                <a href="<?php echo SITE_URL; ?>/admin/hero-banners" class="bg-gray-300 text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-400">
                    Clear
                </a>
            <?php endif; ?>
        </form>
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
        <p class="text-sm text-blue-800">
            <strong>Note:</strong> Only up to 5 active banners will be displayed on the frontend. Banners are ordered by priority (highest first), then display order. Drag and drop to reorder banners.
        </p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <?php if (empty($banners)): ?>
            <div class="px-6 py-12 text-center text-gray-500">
                No banners found. <a href="<?php echo SITE_URL; ?>/admin/hero-banner/add" class="text-pink-600 hover:underline">Add your first banner</a>
            </div>
        <?php else: ?>
            <div id="banners-list" class="divide-y divide-gray-200">
                <?php foreach ($banners as $banner): ?>
                    <div class="banner-item px-6 py-4 hover:bg-gray-50 transition-colors" 
                         data-banner-id="<?php echo $banner['banner_id']; ?>"
                         draggable="true">
                        <div class="flex items-center gap-6">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex flex-wrap gap-3 text-sm text-gray-600">
                                    ...
                                    <span class="flex items-center gap-1">
                                        ...
                                        <span class="px-2 py-1 bg-blue-100 text-blue-800 rounded"><?php echo ucfirst($banner['target_type']); ?></span>
                                    </span>
                                    <span class="flex items-center gap-1">
                                        <span class="font-medium">Device:</span>
                                        <span class="px-2 py-1 bg-purple-100 text-purple-800 rounded"><?php echo ucfirst($banner['device_visibility']); ?></span>
                                    </span>
                            ...
                            <!-- Status Badge -->
                            <div class="flex-shrink-0">
                                <span class="px-3 py-1 rounded-full text-sm font-medium <?php echo $banner['status'] === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'; ?>">
