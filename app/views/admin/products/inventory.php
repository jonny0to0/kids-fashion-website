...
<div class="admin-card mb-6">
    <form method="GET" action="<?php echo SITE_URL; ?>/admin/products/inventory" class="flex flex-wrap gap-4 items-end">
        ...
        <div class="min-w-[150px]">
            ...
            <select name="stock" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent">
                <option value="all" <?php echo ($stockFilter ?? 'all') === 'all' ? 'selected' : ''; ?>>All Products</option>
        ...
        <?php if (!empty($search) || ($stockFilter ?? 'all') !== 'all'): ?>
            <a href="<?php echo SITE_URL; ?>/admin/products/inventory" class="px-6 py-2 border border-gray-300 rounded-lg font-medium text-gray-700 hover:bg-gray-50">
                Clear
            </a>
        <?php endif; ?>
    </form>
                    ...
                    <?php foreach ($products as $product): ?>
                        <tr class="border-b border-gray-100 hover:bg-gray-50">
                            ...
                            <td class="py-3 px-4 text-center">
                                ...
                                ?>
                                <span class="font-semibold <?php echo $currentStock <= 0 ? 'text-red-600' : ($currentStock <= $defaultLowStockThreshold ? 'text-orange-600' : 'text-green-600'); ?>">
                            ...
                            <td class="py-3 px-4 text-center">
                                ...
                                <?php elseif ($currentStock <= $defaultLowStockThreshold): ?>
                                    <span class="px-2 py-1 text-xs font-medium rounded-full bg-orange-100 text-orange-800">Low Stock</span>
                                <?php else: ?>
